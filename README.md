...
jobs:
  build:
    ...
    - uses: ricardodalarme/action-autotag@main
      with:
...
1. `tag_message`
    ...
    changelog will be generated from the commits between the latest tag and the new tag (HEAD). Setting this option will override it with a custom message.
...
1. `tagref`: The reference path of the new tag (e.g., `refs/tags/v1.0.0`).
...
- [**Klemensas**](https://github.com/Klemensas) (Original project creator)
